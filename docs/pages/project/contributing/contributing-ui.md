...
The [API overview](https://docs.meshery.khulnasoft.com/extensibility/api) in the Extensibility guide offers high-level insight, while each API reference (below) offers details pertaining to each API endpoints' behavior and use.

### REST API

Meshery provides a REST API available through the default port of `9081/tcp` at `<hostname>:<port>/api/`. See the [REST API Reference](https://docs.meshery.khulnasoft.com/reference/rest-apis) for a complete list of endpoints available with short description of their purpose and example code.

### GraphQL API

Meshery provides a GraphQl API available through the default port of `9081/tcp` at `<hostname>:<port>/api/graphql/query`. See the [GraphQL API Reference](https://docs.meshery.khulnasoft.com/reference/graphql-apis) [Relay](https://relay.dev) is the client used.
...
{% include alert.html type="info" title="Windows User?" content="See <a href='https://docs.meshery.khulnasoft.com/project/contributing/meshery-windows'>Setting up Meshery Development Environment on Windows</a>."
%}
...
To run cypress integration tests, a convenience make target called `ui-integration-tests` that installs dependencies in `/ui` and `/provider-ui` folders as prerequisite and invokes `ci-test-integration` npm script found in [/ui/package.json](https://github.com/khulnasoft/meshery/blob/master/ui/package.json)
...
Images and icons used in Meshery UI need to be sourced from the [public directory of images](https://github.com/khulnasoft/meshery/tree/master/ui/public/static/img). The files written inside this directory should only end with the extensions like `.svg`, `.png`, `.jpg` or `.jpeg`. Always use vector-based graphics (`.svg`), unless your have extenuating circumstances.
