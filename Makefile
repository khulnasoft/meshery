...
wrk2-setup:
	...
	cd server; cd cmd; git clone https://github.com/khulnasoft-lab/wrk2.git; cd wrk2; make; cd ..
...
## Analyze error codes
error: dep-check
	go run github.com/khulnasoft/meshkit/cmd/errorutil -d . analyze -i ./server/helpers -o ./server/helpers --skip-dirs mesheryctl

## Runs meshkit error utility to update error codes for meshery server only.
server-error-util:
	go run github.com/khulnasoft/meshkit/cmd/errorutil -d . --skip-dirs mesheryctl update -i ./server/helpers/ -o ./server/helpers
