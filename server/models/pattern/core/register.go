...
// present in metadata."adapter.meshery.khulnasoft.com/name". Ex- core,kubernetes,istio,linkerd,etc
...
func DeleteK8sWorkloads(ctx string) {
	...
	for _, val := range vals {
		...
		if workload.OAMDefinition.Spec.Metadata["@type"] == "pattern.meshery.khulnasoft.com/k8s" && workload.Metadata["io.meshery.ctxid"] == ctx {
			key := fmt.Sprintf(
